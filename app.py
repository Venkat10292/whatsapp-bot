...
# Angel One patch for 'proxies' bug
from SmartApi.smartConnect import SmartConnect as RealSmartConnect
sig = inspect.signature(RealSmartConnect.__init__)
if 'proxies' in sig.parameters:
    params = [p for p in sig.parameters.values() if p.name != 'proxies']
    RealSmartConnect.__init__.__signature__ = sig.replace(parameters=params)
